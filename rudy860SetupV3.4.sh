...
doCopyUserApk(){
  ...
  for iapk in $APP_RUDDY; do
      echo "do --> cp $DIR_RUDDY$iapk $DIR_APP"
	  cp $DIR_RUDDY$iapk $DIR_APP
  done
}
...
doCopyDataSetting(){
  ...
  cp -r $DIR_RUDDY"data/com.fikrul.mkctv/" /sdcard/bak/temp/
  cp -r $DIR_RUDDY"b860/com.ss.squarehome2/" /sdcard/bak/temp/
  cp -r $DIR_RUDDY"RuddyB860H.zip" /sdcard/bak/setdata/
}
...
doInjectScript(){
  ...
  # temporary
  cp /sdcard/rudy860SetupV3.4.sh /system/bin/860
  #set permission
